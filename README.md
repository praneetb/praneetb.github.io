...
- [Trips](/trips/) — public index of trip scrapbooks from `_data/trips.yml`. Each trip has its own `public: true|false` flag (not a site-wide gate). Guests see only public trips; private trips appear after the same `site.admin` sign-in. India 2026 is `public: true` (flight seed + cabin media) on `/trips/india-2026/` without a page-gate. Flip a trip’s flag and merge to change guest visibility. Distinct from the countries globe — do not invent itinerary.
...
- [Recipes](/recipes/) — public cook-at-home scrapbook from `_data/recipes.yml`, separate from `/food/` favorites and `/cocktails/` drink videos. Cards, light search, and All / Breakfast chips. First card only: Gourmet Poblano Cream Corn & Sungold Tomato Egg Bites, with a sticky-note still (not laptop/whiteboard chrome). Do not invent recipes.
...
- [Bike](/bike/) — public Softail Slim outing scrapbook from `_data/bike.yml`. Hero plus an extendable `media` list (image or video). First cut uses two Praneet-credited stills under `assets/images/bike/`. Empty `src` placeholders do not render. Do not invent rides, routes, or captions, and do not put bike stock on the public home.
