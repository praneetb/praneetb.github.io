...
- [Food](/food/) — public scrapbook of **favorites only** from `_data/food.yml` (seeded from vault `Favorite Restaurants.md` and `Favorite Cocktails.md`). Khan's Karahi Kabob (Fremont) with starred Goat Peshawari Namakmandi Karahi, plus Rum Buck (pinned) and Long Island Iced Tea linking to `/cocktails/`. No logged-meal fillers, no invented dishes, no non-favorite cocktails.
