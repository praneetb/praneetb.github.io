(function () {
  ...
  function readSortPref() {
    try {
      var stored = localStorage.getItem(SORT_KEY);
      if (stored === "name" || stored === "recent") {
        return stored;
      }
    } catch (err) {}
    return "name";
  ...
  function normalizeEntry(raw) {
    ...
    if (!name && resolved && resolved.name) {
      name = resolved.name;
    }
    if (!name) {
      return null;
    ...
    if (raw.id) {
      ...
      entry.id = code.toLowerCase();
    } else {
      entry.id = name.toLowerCase().replace(/\s+/g, "-");
    }
    if (year) {
      entry.year = year;
    }
  ...
  function visibleCountries() {
    return publicPayload().countries.map(normalizeEntry).filter(Boolean);
  }
